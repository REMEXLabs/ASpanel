...
var AS_Slider= function() {
    ...
    this.init = function(slider_ID) {
        ...
        // init number field values.
        $("#" + this.ui_input_id)
            .attr('step', this.stepSize)
            .attr('min', this.minValue)
            .attr('max', this.maxValue);

        // Set ui-component input element value.
        $("#" + this.ui_input_id).val($("#" + slider_ID).slider("value"));

        // updating slider when changing value of input.
        $("#" + this.ui_input_id).on('change', ev => {
...
////////////////////////////////////////////////
////// AS DROPDOWN WITH BUTTON COMPONENT //////
///////////////////////////////////////////////

/**
 * AS_DropDownWithButton component creates a dropdown menu which shows a button to confirm the selected value in the ASpanel.
 */
var AS_DropDownWithButton = function() {
    this.value = "";
    this.defaultValue = "";
    this.dropdown_ID = "";
    this.ui_component_id = "";
    ...
    this.reset = function() {
        ...
        // hiding button
        $("#" + this.dropdown_ID + "-button").attr('hidden', true);
    };
    ...
    this.init = function(dropdown_ID) {
        ...
        $("#" + dropdown_ID).change( function() {
            ...
            // showing button
            $("#" + dropdown_ID + "-button").removeAttr('hidden');
        });

        $("#" + dropdown_ID + "-button").on('click', function() {
            UIComponentList[dropdown_ID].clickEvent($("#" + dropdown_ID).val());
        });
...
///////////////////////////////////
////// AS COLOR PICKER COMPONENT //////
///////////////////////////////////

/**
 * AS_ColorPicker component creates a color picker in the ASpanel.
 */
var AS_ColorPicker = function() {
    this.value;
    this.defaultValue = '#ffff80';
    this.colorPicker_ID = "";
    this.ui_component_id = "";


    this.reset = function() {
        console.log("Reset " + this.colorPicker_ID);
        this.value = this.defaultValue;
        $("#" + this.colorPicker_ID).val(this.value);
        UIComponentList[this.colorPicker_ID].changeEvent(this.value);
        console.log("Value of " + this.colorPicker_ID + " is set to " + this.getValue());
    };

    /**
     * Initialize component. Sets object parameter and applies eventhandler.
     * @param {string} colorPicker_ID ID of the HTML container used for the component.
     */
    this.init = function(colorPicker_ID) {
        // Get values from UIComponentList.
        this.defaultValue = UIComponentList[colorPicker_ID].defaultValue;
        this.value = this.defaultValue;
        this.colorPicker_ID = colorPicker_ID;
        this.ui_component_id = UIComponentList[colorPicker_ID].ui_component_id;

        // Init color picker
        $("#" + colorPicker_ID).change(function() {
            console.log("Color Picker " + colorPicker_ID + " clicked");
            UIComponentList[colorPicker_ID].changeEvent($(this).val());
            this.value = $(this).val();

        });

        $("#" + colorPicker_ID).val(this.value);

        // Run change function one time for initialisation.
        UIComponentList[colorPicker_ID].changeEvent(this.value);
    };

     /**
     * Set the color picker to value.
     */
    this.setValue = function(value) {
        this.value = value;
        $("#" + this.colorPicker_ID).val(this.value);
        UIComponentList[this.colorPicker_ID].changeEvent(value);
        console.log("Value of " + this.colorPicker_ID + " is set to " + this.getValue());
    };

    this.getValue = function() {
        return $("#" + this.colorPicker_ID).val();
    };

    this.overwriteDefault = function () {
        //console.log("Overwrite default value of " + this.checkbox_ID);
        //this.defaultValue = UIComponentList[this.checkbox_ID].defaultValue;
        this.value = UIComponentList[this.colorPicker_ID].defaultValue;
        $("#" + this.colorPicker_ID).val(this.value);
        UIComponentList[this.colorPicker_ID].changeEvent(this.value);
    };
};
