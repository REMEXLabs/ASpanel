...
const myURCLightController = (function () {

    /* --------- Private Properties --------- */
    const module = {};

    const privatPoperties = {};
    ...
    /**
     * Create a sign language video request
     * @param slSettings contains language, interpreter nam, interpreter type and content of the signlanguage video.
     */
    module.createSLVRequest = function (slSettings) {
        let requestURL = "";

        const baseURL = "http://res.openurc.org/retrieve?name=http%3A%2F%2Fhdm-stuttgart.de%2Fcloud4all%2FBanking%2F";

        const urlstr_Content = slSettings.Content;
        const urlstr_InterpreterName = "_" + slSettings.InterpreterName;
        const urlstr_InterpreterType = "_" + slSettings.InterpreterType;
        let urlstr_Language = "_" + slSettings.Language;

        const str_mimeType = ".webm";

        if (urlstr_Language === "_asl") {
            urlstr_Language = "";
        }

        requestURL = baseURL + urlstr_Content + urlstr_InterpreterName + "_" + "medium" + urlstr_Language + str_mimeType;
        return requestURL;

    };

    /**
     * Locks or unlocks Interpreter options after choosing a sign language language
     */
    module.lockSLVOptions = function (slLanguage) {

        console.log(slLanguage);

        if (slLanguage === "gsg") {
            $("#myASpanel-iFrame").contents().find("#signlanguageInterpreterName_dropdown option").siblings().removeAttr("selected");
            $("#myASpanel-iFrame").contents().find("#signlanguageInterpreterName_dropdown option").siblings().removeAttr("disabled");

            // deactivate not supported options.
            $("#myASpanel-iFrame").contents().find("#signlanguageInterpreterName_dropdown option[value='Tony'], #signlanguageInterpreterName_dropdown option[value='Maria']").attr("disabled", true);
            //.siblings().removeAttr("disabled");
            // select supported option
            $("#myASpanel-iFrame").contents().find("#signlanguageInterpreterName_dropdown option[value='Feldmann']").attr("selected", "selected");
            slSettings.InterpreterName = "Feldmann";

        }
        else if (slLanguage === "asl") {
            $("#myASpanel-iFrame").contents().find("#signlanguageInterpreterName_dropdown option").siblings().removeAttr("selected");
            $("#myASpanel-iFrame").contents().find("#signlanguageInterpreterName_dropdown option").siblings().removeAttr("disabled");

            // deactivate not supported options and activate supported.
            $("#myASpanel-iFrame").contents().find("#signlanguageInterpreterName_dropdown option[value='Feldmann']").attr("disabled", true);
            // select supported option
            $("#myASpanel-iFrame").contents().find("#signlanguageInterpreterName_dropdown option[value='Maria']").attr("selected", "selected");
            slSettings.InterpreterName = "Maria";
...
// function createSLVRequestURL(slSettings) {
    // var requestURL = "";
//
    // var baseURL = "http://res.openurc.org/retrieve?name=http%3A%2F%2Fhdm-stuttgart.de%2Fcloud4all%2FBanking%2F";
//
//
//
//
    // requestURL = baseURL + slSettings.Content + "_" + slSettings.InterpreterName + "_"+ "medium" + ".webm";
//
    // return requestURL;
//
//
...
const cookie_function_filepath = '../../../../js/jquery-cookie-master/jquery.cookie.js';


const URCLightController = {
...
function getMarkers() {
    ...
    const c = JSON.stringify(markerObject);
...
function getAvailableValues(settings, callback, value) {
    ...
    const markerObject = $.parseJSON($.cookie('sign-language-markers'));
    ...
    const markers = markerObject.markers;

    // if empty
	if (!markers[0]) return;

	//var label = marker[0];
    const label = markers[0];
...
//		this.includesAudio[1]	= settings.playAudio;
	...
	search : function() {
		...
		// using the proxy
        const uri = "http://" + this.serveruri + "query?" + this.get;
        console.log(uri);
	...
	parseURCXMLForLanguages : function(xml, cb_) {
		...
		$(xml).find("resource").each(
			function()
			{
                ...
                const l = $(this).find("prop[name='http://purl.org/dc/elements/1.1/language']").attr("val");
...
//
	...
	parseURCXMLForInterpreterTypes : function(xml, cb_) {
		...
		$(xml).find("resource").each(
			function()
			{
                const t = $(this).find("prop[name='http://openurc.org/ns/res#interpreterType']").attr("val");
	...
	parseURCXMLForInterpreterNames : function(xml, cb_) {
		...
		$(xml).find("resource").each(
			function()
			{
                const t = $(this).find("prop[name='http://openurc.org/ns/res#interpreterName']").attr("val");
	...
	parseURCXMLResponse	: function(xml) {
		...
		//
        let vName = $(xml).find("resource:first")
            .find("prop[name='http://openurc.org/ns/res#filename']")
            .attr("val");
        vName = vName.split(".");
		vName = vName[0];

        let elUrl = $(xml).find("resource:first")
            .find("globalAt")
            .text();
